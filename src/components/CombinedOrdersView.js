...
const CombinedOrdersView = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async (phoneNumberToFetch) => {
      try {
        setLoading(true);
        setError('');
        ...
        if (response.data.success) {
          ...
          categorizeOrders(fetchedOrders);
    ...
    socket.on('orderStatusUpdate', (updatedOrder) => {
      if (updatedOrder.phoneNumber === storedPhoneNumber) {
  ...
  const handleSubmitFeedback = async (orderId) => {
    ...
    if (!feedbackData || !userPhoneNumber) return;
                    ...
                    <div className="flex-shrink-0">
                        <span className={`px-4 py-2 rounded-full text-sm font-bold 
                            ${order.cookStatus === 'Served' ? 'bg-purple-100 text-purple-800' :
                               order.cookStatus === 'Cancelled' ? 'bg-red-100 text-red-800' :
